...
class PDFExportService:
    ...
    def generate_pdf(
        self, resume: JSONResume, output_path: Path, template: str = "default"
    ) -> Path:
        ...
        for job in work_experiences:
            ...
            if hasattr(job, "start_date") or hasattr(job, "end_date"):
                date_range = self._format_date_range(
                    getattr(job, "start_date", None), getattr(job, "end_date", None)
                )
                if date_range:
    ...
    def _add_education(self, story, education_list):
        ...
        for edu in education_list:
            ...
            if hasattr(edu, "start_date") or hasattr(edu, "end_date"):
                date_range = self._format_date_range(
                    getattr(edu, "start_date", None), getattr(edu, "end_date", None)
                )
                if date_range:
