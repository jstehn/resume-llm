...
from ...database.models import JobApplication as DBJobApplication, User as DBUser, ResumeVersion as DBResumeVersion
from ...models.job import JobApplication, JobApplicationCreate, JobApplicationUpdate, JobAnalysisRequest, JobAnalysisResponse
...
@router.post("/applications", response_model=JobApplication, status_code=status.HTTP_201_CREATED)
async def create_job_application(
    ...
    user = db.query(DBUser).filter(DBUser.id == application.user_id).first()  # Fixed: Added user_id to model
    if not user:
        raise HTTPException(
    ...
    resume_version = db.query(DBResumeVersion).filter(
        DBResumeVersion.id == application.resume_version_id
    ).first()
...
async def get_user_job_applications(user_id: int, db: Session = Depends(get_db)):
    ...
    applications = db.query(DBJobApplication).filter(
        DBJobApplication.user_id == user_id
    ).all()
...
async def get_job_application(application_id: int, db: Session = Depends(get_db)):
    ...
    application = db.query(DBJobApplication).filter(
...
async def analyze_job_resume_match(
    ...
    try:
        ...
        return {
            ...
            "optimized_resume": result.get("optimized_resume").model_dump() if result.get("optimized_resume") else None
        }
